...
/** IA прототипу v1 (оригінальний порядок секцій) */
export const NAV_SECTIONS_V1 = [
...
/**
 * IA прототипу v2: додано верхній пункт «Merchant insights»,
 * інший порядок блоків Reports / Taxes (порівняння ітерацій).
 */
export const NAV_SECTIONS_V2 = [
  { type: "item", id: "dashboard", label: "Dashboard", icon: "dashboard" },
  { type: "item", id: "merchant-insights", label: "Merchant insights", icon: "reportsExports" },
  {
    type: "group",
    id: "payments",
    label: "Payments",
...
/** Зворотна сумісність */
export const NAV_SECTIONS = NAV_SECTIONS_V1;

export function isValidNavId(sections, id) {
  if (!id) return false;
  for (const s of sections) {
    if (s.type === "item" && s.id === id) return true;
    if (s.type === "group" && s.children.some((c) => c.id === id)) return true;
  }
  return false;
}
